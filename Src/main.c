...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
//#define GREEN_GPIO GPIOD
//#define GREEN_PIN GPIO_PIN_2
//#define RED_GPIO GPIOD
//#define RED_PIN GPIO_PIN_4
//#define BLUE_GPIO GPIOD
...
void UART_Receive_Message(UART_HandleTypeDef *huart, char buffer[MAX_BUFFER_SIZE]);
/* USER CODE END PFP */
...
void UART_Receive_Message(UART_HandleTypeDef *huart, char buffer[MAX_BUFFER_SIZE]) {
    ...
    while (1) {
        ...
        // Break if termination character is received or buffer is full
        if (temp == '\r' || temp == '\n' || temp == '\0' || index >= MAX_BUFFER_SIZE - 1) {
            buffer[--index] = '\0';  // Null-terminate the string
            break;
        }
    }

    // Echo received message back via UART
...
int main(void)
{
  ...
  char recievedMessage[MAX_BUFFER_SIZE];
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    UART_Receive_Message(&huart3, recievedMessage);
    if (!strcmp(recievedMessage, "green")) {
    	// flashing green means success
      leds_off();
      HAL_TIM_Base_Start_IT(&htim2);
      ...
      HAL_UART_Transmit(&huart3, "red", 3, HAL_MAX_DELAY);
      HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY);
    } else if (!strcmp(recievedMessage, "blue")) {
    	// solid blue means manual mode
      leds_off();
      HAL_GPIO_WritePin(BLUE_GPIO, BLUE_PIN, GPIO_PIN_SET);
      HAL_UART_Transmit(&huart3, "blue", 4, HAL_MAX_DELAY);
      HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY);
    } else if (!strcmp(recievedMessage, "off")) {
    	// turn off everything
      leds_off();
      HAL_UART_Transmit(&huart3, "off", 3, HAL_MAX_DELAY);
      HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY);
    } else{
    	// unexpected command, do nothing
		HAL_UART_Transmit(&huart3, "error", 3, HAL_MAX_DELAY);
		HAL_UART_Transmit(&huart3, "\r\n", 2, HAL_MAX_DELAY);
    }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM2) {
      HAL_GPIO_TogglePin(GREEN_GPIO, GREEN_PIN); // Toggle LED (PA3)
  }
}
/* USER CODE END 4 */
